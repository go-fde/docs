...
`github.com/go-fde/luks` is pure-Go **read/write** support for LUKS1 and LUKS2
full-disk-encryption containers. The on-disk header is big-endian; containers
created or opened by this library are interoperable with `cryptsetup` for the
...
| Integrity (AEAD per-sector tags, `--integrity`) | ❌ (complex; out of scope) |
| Hardware tokens (FIDO2, TPM, smart cards) | ❌ (out of scope for a passphrase library) |
...
independently of where the payload sits in the file. This matches `cryptsetup`,
making containers created by this library interoperable with the standard Linux
tooling.

## Compatibility

Containers created or opened by this library are fully interoperable with
`cryptsetup` for the supported feature set, which covers the vast majority of
LUKS containers found in practice:
...
`OpenFrom` layers LUKS on top of any read-write-closable block device instead of
a plain file — useful when the container lives inside a virtual disk image such
as QCOW2. The caller passes any value satisfying:
...
When the returned `*Device` is closed it also closes the underlying block
device.
